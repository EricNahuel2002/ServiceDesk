...
public sealed class WorkFinishedEmailFunctionTests
{
    private readonly FakeTicketRepository _tickets = new();
    private readonly FakeEmailService _email = new();
    private readonly WorkFinishedEmailFunction _function;

    public WorkFinishedEmailFunctionTests()
    {
        _function = new WorkFinishedEmailFunction(
            _tickets,
            _email,
            NullLogger<WorkFinishedEmailFunction>.Instance);
    }
    ...
    public async Task Run_SendsEmailToRequester()
    {
        ...
        {
            EventType = NotificationEvents.WorkFinished,
